...
void parse_var_assignation(ExprContainer *container, const ExprData *expression){
    ...
    search_variable(left_assignation->expr, container->function->paramList, container->scope);
    store_register_in_expression(container, left_assignation, register_name);

    ExprData *stack_expr = ExprData_stack(left_assignation);
    stack_expr->type = right_type;
    stack_expr->is_func_argument = expression->is_func_argument;
...
void parse_func_call(ExprContainer *container, ExprData *func_call_expr){
    ...
    if(function_called->type != void_type){
        store_register_in_stack(container, register_name);
        ExprData *stack_expr = ExprData_stack(func_call_expr);
        stack_expr->is_func_argument = func_call_expr->is_func_argument;
        stack_expr->type = get_ExprType_const(function_called->type);
        ExprVector_endadd(container->exprs_stack, stack_expr);
    }
}
...
void parse_return(ExprContainer *container, ExprData *return_type_expr){
    ...
    if(return_type_expr->type == expr_return_expr){
        ...
        if(return_type != expected_type){
            error_wrong_return_type(container->function->name, ExprTypeEnum_name(expected_type), ExprTypeEnum_name(return_type));
        }
...
void parse_arithmetic(ExprContainer *container, ExprData *arithmetic_expr){
    const char *register_name = "r13";
    store_arithmetic_in_stack(container, arithmetic_expr, register_name);
}

void parse_comparation(ExprContainer *container, ExprData *comparation_expr){
    const char *register_name = "r13";
    store_comparation_in_stack(container, comparation_expr, register_name);
}
    ...
    for(int i = 0; i < ExprVector_len(exprs_vec); ++i){
        ...
        switch(expr->type){
            case expr_scope:{
                    ...
                    parse_expressions(output_file, function, next_scope);
