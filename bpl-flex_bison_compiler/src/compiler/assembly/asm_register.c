...
ExprTypeEnum load_const_expr_into_register(ExprContainer *container, ExprData *expression, const char *register_name){
    assert(ExprTypeEnum_is_const(expression->type));
...
ExprTypeEnum load_stackexpr_into_register(ExprContainer *container, ExprData *expression, const char *register_name){
    assert(container->stack_size > 0);
    const char *expr_name = expression->expr;
    const char *asm_str = "\tpopq\t%%%s # %s \n";
    fprintf(container->out_file, asm_str, register_name, expr_name);
...
ExprTypeEnum load_expression_into_register(ExprContainer *container, ExprData *expression, const char *register_name){
    if(expression->value_in_stack){
        return load_stackexpr_into_register(container, expression, register_name);
    }
